...
### parse_code
- 通过parse_code工具快速分析有关的项目语法树，了解代码整体框架。在完整读取文件之前，对文件的语法树进行大体了解，并根据返回的语法树中的行号范围等信息，对感兴趣区域、可能对功能开发有价值的部分信息进行精准读取，防止读取过多内容污染上下文资源。

### search_in_code
- 通过search_in_code查找特定的类、结构体、成员（变量、函数、方法）等信息，快速定位所有需要修改的地方，支持同时查询多个关键字。
...
2. 对于查找一个关键字的返回结果，匹配到的并不一定指的是相同的东西。例如，查找关键字"obj"，结果中，在某个子树中它可能是一个定义的类，在另一个子树中它可能是一个函数名，而又在另一个类中，他可能是一个字符串"obj"，所以需要根据返回的结果语法树，结合读取源代码，做进一步分析，判断是否是需要搜索的目标。
3. 经过语法树分析后，如果需要阅读源文件，在完成需求的前提下，应尽可能通过语法树中提供的行号，精确定位到完成需求所需的部分文件，仅读取部分行，减少阅读的源文件的规模。
4. 在通过语法树获取行号范围后，如果要以该行号范围为依据对文件进行读取，应向行号范围前后多读几行，以确保代码块前后相应的注释部分也能被读到。
